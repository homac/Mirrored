...
public class ArticlesList extends ListActivity implements Runnable {
    ...
    protected void onCreate(Bundle icicle) {
        try {
            ...
            // check if we're coming from the feedCategory view and/or have a preference feedCategory
            if (getIntent().hasExtra(app.EXTRA_CATEGORY)) {
                ...
                if (startWithCategory != null && startWithCategory.length() != 0
                    ...
                    if (MDebug.LOG)
                        Log.d(TAG, "Got feedCategory from preferences: " + startWithCategory);

                    _url = new URL(BASE_CATEGORY_FEED + startWithCategory);
                    category = startWithCategory;
                } else {
                    if (MDebug.LOG)
                        Log.d(TAG, "No feedCategory set, using BASE_FEED: " + BASE_CATEGORY_FEED);
                    _url = new URL(BASE_CATEGORY_FEED);
                }
            }
            ...
            if (!_internetReady) {
                ...
                if (!app.getBooleanPreference("PrefStartWithOfflineMode", false)) {
                    Toast.makeText(getApplicationContext(),
                            R.string.switch_to_offline_mode,
                            Toast.LENGTH_LONG).show();
                }
            }
            setTitle(title);

            _pdialog = ProgressDialog.show(this, "", getString(R.string.progress_dialog_load),
                    true, false);
    ...
    public void run() {
        ...
        if (_saveAllArticles) {
            ...
            ArticleContentDownloader downloader =
                    new ArticleContentDownloader(app, _displayMetrics(), _articles,
                            true, false, _internetReady);
            downloader.download();
        ...
        if (getIntent().hasExtra(app.EXTRA_CATEGORY)) {
            String category = getIntent().getExtras().getString(app.EXTRA_CATEGORY);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        menu.add(Menu.NONE, MENU_CATEGORIES, Menu.NONE, R.string.menu_categories)
        ...
        if (_internetReady) {
            ...
            if (_articles.size() > 0)
                menu.add(Menu.NONE, MENU_DELETE_ALL, Menu.NONE, R.string.menu_delete_all)
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case MENU_SAVE_ALL:
                ...
                if (_saver.storageReady()) {
                    _pdialog = ProgressDialog.show(this, "",
                            getString(R.string.progress_dialog_save_all),
                            true, false);
                    _saveAllArticles = true;
                    Thread thread = new Thread(this);
                    thread.start();
                } else {
                    app.showDialog(ArticlesList.this, getString(R.string.error_saving));
                }
            ...
            case MENU_DELETE_ALL:
                ...
                // delete articles from last to first and don't use for loop or iterator to prevent
                // ConcurrentException
            ...
            case MENU_ONLINE_MODE:
                ...
                if (!app.online())
                    app.showDialog(this, getString(R.string.please_check_internet));
                else {
                    intent = new Intent(this, ArticlesList.class);
                    intent.setAction(Intent.ACTION_VIEW);
                    startActivity(intent);
