...
//        InputStream i = null;
        ...
        try {
            ...
            if (online) {
                ...
                tParser.parse(new ByteArrayInputStream(feedString.getBytes()), this);
            } else {
    ...
    public void endElement(String uri, String name, String qName)
        ...
        if (name.trim().equals("title")) {
            ...
            // Subfeeds e.g. netzwelt oder kultur have no category tag, so calculate it from url
            if (_currentArticle.feedCategory == null || _currentArticle.feedCategory.length() == 0) {
                Log.w(TAG, "category of " + _currentArticle.title + " ist empty");
                _currentArticle.feedCategory = feedCategory;
            }
            //feedCategory;
            // Check if looking for article, and if article is complete
            if (_currentArticle.url != null && _currentArticle.title.length() > 0 && _currentArticle.description.length() > 0) {
                // ugly hack: We currently don't support "Fotostrecke and Videos:", so don't add article if it is one
                if (_currentArticle.url != null && !_currentArticle.url.toString().contains("/fotostrecke/") && !_currentArticle.url.toString().contains("/video/")) {
